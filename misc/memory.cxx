 ...
 * Last edited on February 07 of 2021 at 18:01 BRT */
...
Void CopyMemory32(Void *Buffer, const Void *Source, UIntPtr Length) {
	if (Buffer == Null || Source == Null || Buffer == Source || !Length) {
		return;
	}
...
Void SetMemory32(Void *Buffer, UInt32 Value, UIntPtr Length) {
	...
	for (UInt32 *buf = reinterpret_cast<UInt32*>(Buffer); Length; Length--) {
		*buf++ = Value;
	}
}
