...
Void ByteHelper::Convert(Float Value, Char &Unit, UIntPtr &Left, UIntPtr &Right) {
	 ...
	 * can represent it in a more human-readable way. */
	...
	/* To get the left side, just convert the value into an integer, to get the (two first) numbers
	 * after the floating point, subtract the left value we got from the float value, multiply the result
	 * by 10*<how many number after the floating point we want>, and convert the result into an integer. */
