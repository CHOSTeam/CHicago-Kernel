 ...
 * Last edited on November 28 of 2020, at 13:21 BRT */
...
/* Comparing floats by just doing A == B (or A != B) is dangerous, instead, you should do abs(A - B)
 * and compare if the value is lower/greater than the epsilon. */
...
#ifdef ARCH_64
	static inline IntPtr Abs(IntPtr Value) { return __builtin_llabs(Value); }
#else
	static inline IntPtr Abs(IntPtr Value) { return __builtin_abs(Value); }
#endif
#ifdef __SSE__
	static inline IntPtr Round(Float Value) { IntPtr ret; asm volatile("cvtsd2si %1, %0" : "=r"(ret) : "xm"(Value));
											  return ret; }
#else
