 ...
 * Last edited on August 22 of 2020, at 11:47 BRT */
...
/* The SIA file format is what we use for storing the initrd (that is, the drivers and the some other arch-specific things).
 * While we can load a SIA file from some already loaded filesystem, we're probably going to need to load it from the
 * memory (for the initrd), so we need some way to access the memory as if it was a file. This "MemFs" is not avaliable
 * as an actual FS, so you can't just ::Mount some random virtual memory location. */
...
class SiaFs {
private:
	/* All the headers are directly copied from the sia.h file (of the x86 bootloader), except that it is adapted to C++
	 * and to the CHicago type naming system... except for the PrivData struct, this was created only for the kernel SIA
...
public:
	/* Ok, I know what you're thinking, that the ::Header field is private and etc, but this constructor is only supposed
	 * to be called by the Mount function (and by the MountRamFs function), so we don't care about the user having problems
	 * with it. */
...
private:
	...
	/* Finally, some non-static functions, they are used for allocating new file/data entries, we could implement them
	 * as static functions, but it's easier to just do this. */
