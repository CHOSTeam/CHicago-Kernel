 ...
 * Last edited on August 07 of 2020, at 21:07 BRT */
...
Void DebugImpl::WriteInt(Char Value) {
	/* Before sending the data, make sure that the port isn't busy doing something else (like sending other data, or
	 * receiving data). */
...
Void InitDebugInterface(Void) {
	/* COM1 is at the IO port 0x3F8, before using it, we need to configure it, first, set the divisor to 1 (115200bits/s). */
