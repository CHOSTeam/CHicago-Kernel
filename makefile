...
# Last edited on January 20 of 2020, at 11:06 BRT
...
ifeq ($(ARCH),x86)
	...
	ARCH_OBJECTS := $(addprefix build/$(ARCH)_$(SUBARCH)/, $(addsuffix .o, $(shell find arch/$(ARCH) -name "*.c" | grep -v **$(NOT_SUBARCH).* | sed 's|^arch/$(ARCH)|arch|') $(shell find arch/$(ARCH) -name "*.s" | grep -v **$(NOT_SUBARCH).* | sed 's|^arch/$(ARCH)|arch|')))
	OBJCOPY_ARCH := i386
...
OBJECTS := $(addprefix build/$(ARCH)_$(SUBARCH)/, $(addsuffix .o, $(shell find . -name '*.c' | grep -v arch | sed 's|^./||')))
OTHER_OBJECTS := $(addprefix build/$(ARCH)_$(SUBARCH)/, $(addsuffix .oo, font.psf splash.bmp))

ifeq ($(wildcard sc/sctable.c),)
	OBJECTS += build/$(ARCH)_$(SUBARCH)/sc/sctable.c.o
endif
...
build/$(ARCH)_$(SUBARCH)/arch/%.s.o: arch/$(ARCH)/%.s
ifeq ($(UNSUPPORTED_ARCH),true)
	$(error Unsupported architecture $(ARCH), subarch $(SUBARCH))
endif
	$(NOECHO)echo Compiling $<
...
build/$(ARCH)_$(SUBARCH)/arch/%.c.o: arch/$(ARCH)/%.c
ifeq ($(UNSUPPORTED_ARCH),true)
	$(error Unsupported architecture $(ARCH), subarch $(SUBARCH))
endif
	$(NOECHO)echo Compiling $<
