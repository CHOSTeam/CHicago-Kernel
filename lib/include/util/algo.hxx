...
template<class T, class U> Void InsertionSort(T *Start, T *End, U Compare) {
    /* It's like bubble sort, but a bit faster (and that is why we don't even have a BubbleSort function). */

    for (T *i = Start + 1; i < End; i++) {
        for (T *j = i; j > Start && Compare(*j, j[-1]); j--) Swap(*j, j[-1]);
    }
}

template<class T, class U> static inline Void MakeHeap(T *Start, UIntPtr Root, UIntPtr End, U Compare) {
    ...
    while (2 * Root + 1 <= End) {
        ...
        if (i + 1 <= End && Compare(Start[i], Start[i + 1])) i++;
        if (Compare(Start[Root], Start[i + 1])) Swap(Start[Root], Start[i]);

        Root = i;
    }
}

template<class T, class U> Void HeapSort(T *Start, T *End, U Compare) {
    ...
    if (Start >= End || End - Start <= 2) {
        if (Start < End && End - Start == 2 && Compare(Start[1], Start[0])) Swap(Start[0], Start[1]);
        return;
    }
    ...
    for (IntPtr i = (End - Start - 2) / 2; i >= 0; i--) MakeHeap(Start, i, End - Start - 1, Compare);
...
template<class T, class U> T *SortPartition(T *Start, T *End, U Compare) {
...
template<class T, class U> static inline Void Sort(T *Start, T *End, UIntPtr Depth, U Compare) {
    ...
    while (Start < End) {
        ...
        if (End - Start <= 2) {
            if (End - Start == 2 && Compare(Start[1], Start[0])) Swap(Start[0], Start[1]);
            return;
        } else if (!Depth--) {
...
template<class T, class U> Void Sort(T *Start, T *End, U Compare) {
