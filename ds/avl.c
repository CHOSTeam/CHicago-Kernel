...
// Last edited on January 21 of 2020, at 22:51 BRT

#include <chicago/alloc.h>
#include <chicago/avl.h>

PAvlTree AvlCreate(Int (*compare)(PVoid, PVoid), Boolean free) {
	if (compare == Null) {																		// Make sure that the compare function isn't just a Null pointer
		return Null;
	}
	...
	avl->free = free;
	avl->compare = compare;																		// And let's set the compare function...
...
static PAvlNode AvlRemoveInt(PAvlNode node, PVoid key, Int (*compare)(PVoid, PVoid), Boolean free) {
	...
	if (res > 0) {																				// Our key is bigger than the current node key?
		node->left = AvlRemoveInt(node->left, key, compare, free);								// No...
		goto c;
	} else if (res < 0) {
		node->right = AvlRemoveInt(node->right, key, compare, free);							// Yeah, it is!
		goto c;
	} else if (node->left == Null && node->right == Null) {										// We don't have any children?
		if (free) {																				// Yeah, we don't... just free everything and return..
			MemFree((UIntPtr)node->key);
			MemFree((UIntPtr)node->value);
		}
	...
	node->right = AvlRemoveInt(node->right, min->key, compare, False);							// And remove that successor...
...
Void AvlRemove(PAvlTree avl, PVoid key) {
	...
	avl->root = AvlRemoveInt(avl->root, key, avl->compare, avl->free);							// And redirect to the internal function
}
