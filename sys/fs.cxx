...
/* We need to handle the reference count on all the constructors except for the Void one. We're not going to
 * implement a const File* constructor, instead, call the construtor passing *ptr. */

File::File(Void) : Name("<unknown>"), Flags(0), Fs(Null), Priv(Null), INode(0), References(Null) { }

File::File(const String &Name, UInt8 Flags, const FsImpl *Fs, UInt64 Length, const Void *Priv, UInt64 INode) :
		   Name((String&)Name), Flags(Flags), Fs(Fs), Length(Length), Priv(Priv), INode(INode), References(new UIntPtr) {
	if (References != Null) {
		(*References)++;
	}
}

File::File(const File &Source) : Name(Source.Name), Flags(Source.Flags), Fs(Source.Fs), Length(Source.Length),
								 Priv(Source.Priv), INode(Source.INode), References(Source.References) {
	if (References != Null) {
		(*References)++;
	}
}
...
/* The File class stores all the private data that we need, we could have a function that return the FsImpl,
 * and make some kind of wrapper in the FileSys class (or even just let the user call the functions manually),
 * but it's better to implement said wrappers on the File class itself. */
...
Status File::ReadDirectory(UIntPtr Index, String &Name) const {
	Char *out = Null;
	Status status = (!(Flags & OPEN_DIR) || !(Flags & OPEN_READ) || Fs == Null || Fs->ReadDirectory == Null)
...
Status File::Search(const String &Name, UInt8 Flags, File &Out) const {
	/* ->Search returns the Priv and INode values for the file, we should mount the wrapper around the returned
	 * values, remembering that the Fs remains the same. */
	
	Void *priv;
	UInt64 inode, len;
	
	Status status = (!(this->Flags & OPEN_DIR) || !(this->Flags & OPEN_READ) || Fs == Null || Fs->Search == Null)
					? Status::Unsupported : Fs->Search(Priv, INode, Name.ToCString(), &priv, &inode, &len);
...
Status File::Unmount(Void) const {
	/* Only the FileSys::Unmount function should call us, we don't have any way to be sure that we aren't being
	 * called by something else, but let's at least try to check the name, and if this is an directory. */
	
	return (!Name.Compare("/") || !(Flags & OPEN_DIR) || Fs == Null || Fs->Unmount == Null)
		   ? Status::Unsupported : Fs->Unmount(Priv, INode);
}

/* The Mount/Unmount function should never be called by anyone except the FileSys ::Mount/::Unmount functions,
 * so we don't need to implement the wrappers around them (they are not even specific to one file). */

List<String> FileSys::TokenizePath(const String &Path) {
	/* The String class already have a function that tokenizes the string, and return a List<> of tokens, but
	 * we have some tokens that we need to remove... */
	...
	for (UIntPtr i = 0; i < ret.GetLength();) {
		...
		if (val.Compare(".")) {
			...
			/* '..' means the parent directory, we need to remove ourselves and the token before us (or, in
			 * case there is no entry before us, only remove ourselves. */
...
String FileSys::CanonicalizePath(const String &Path, const String &Increment) {
	/* This time, we need to tokenize TWO strings, append the contents of the second list to the first, and do the
	 * same process we did in the TokenizePath function. */
	...
	for (UIntPtr i = 0; i < lpath.GetLength();) {
		...
		if (val.Compare(".")) {
			...
			/* '..' means the parent directory, we need to remove ourselves and the token before us (or, in case
			 * there is no entry before us, only remove ourselves. */
	...
	/* Finally, let's try to alloc the return string, let's just hope that the allocations are not going to fail
...
Status FileSys::Register(const FsImpl *Info) {
	/* Almost all of the Impl fields can be Null, except for the Name field, as we use it to make sure there is no
	 * duplicate filesystem registered. */
...
Status FileSys::CheckMountPoint(const String &Path) {
	/* We could probably only return a Boolean, BUT, we need to check if the Path starts with a slash, and in the case
	 * have trailing slashes, we need to allocate memory, both return different status codes, so just a Boolean isn't
	 * enough. */
...
Status FileSys::CreateMountPoint(const String &Path, const File &Root) {
	...
	if (path == Null) {
		...
		/* The mount point should never be created with trailing slashes, as the GetMountPoint always removes them
		 * (which we're going to do as well). */
...
Status FileSys::Open(const String &Path, UInt8 Flags, File &Out) {
	 ...
	 * create the file in case it doesn't exists, if we can create all the folders in a recur way etc. We can extract
	 * the valid File flags by using a mask (which zeroes out all the invalid flags).
	 * We need to get the mount point of the path, the GetMountPoint function returns both the mount point and the
	 * remainder of the path (that is, the original path, but with the mount point path removed), we can tokenize the
	 * remainder, and try to open/create everything. */
	...
	/* Now only the creation of the file/directory itself is left, we can remove the last list entry, delete the list,
	 * and try to search for the file. If we do find it, we need to make sure that the user didn't said that we should
	 * only try to create the file, and if we don't find it and the create flag is set, we need to try creating it. */
...
Status FileSys::Mount(const String &Dest, const String &Source, UInt8 Flags) {
	/* We removed the option to manually specify the file system type, this makes the code a bit smaller, and we have less
	 * things to do, we may add it again later, but for now just do the same checks that we did in the old code (dest doesn't
	 * exist, source exists and we can do whatever the Flags say, etc).
	 * The Mount function should clone the file that we pass (by casting it to File* and calling the copy constructor, when
	 * the filesys is implemented on the kernel itself, or by calling the CloneFile function, when the filesys is a driver. */
	...
	for (const FsImpl *fs : FileSystems) {
		if ((status = fs->Mount((Void*)srcp, &priv, &inode)) == Status::Success) {
			...
			/* I think that it is a good idea to fail in the case the FS driver returns that the kernel is out of memory. */
			delete srcp;
			return status;
		}
	}
...
Status FileSys::Unmount(const String &Path) {
	/* Unmounting is just a matter of finding the mount point struct that points to Path (Path has to be the EXACT mount path,
	 * not some sub-folder or file inside the mount point). We need to do the same handling of trailing slashes on the Path as
	 * we do on the CreateMountPoint function. */
...
const MountPoint *FileSys::GetMountPoint(const String &Path, String &Remain) {
	/* We have two options to iterate through the list and try to get the right mount point: checking each mount
	 * point that the path is equal to the start of our Path, and return the one with the bigger length, or, copying
	 * the string, and doing something like what we do at CreateMountPoint, but remembering to save the length, and
	 * only do the 'stop at next slash' after each iteration. We're going with the second option. */
