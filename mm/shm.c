...
// Last edited on January 27 of 2020, at 22:39 BRT
...
Status ShmCreateSection(UIntPtr size, PUIntPtr key, PUIntPtr ret) {
	...
	Status status = MmAllocAddress(L"Shared Memory", size, SHM_NEW_FLAGS, &msect->virt);											// Alloc the virtual address
	...
	for (UIntPtr i = msect->virt; i < msect->virt + size; i += MM_PAGE_SIZE) {														// Let's add everything to the physical regions map
		if (!ListAdd(&msect->shm->pregions, (PVoid)MmGetPhys(i))) {
			...
			MmUnmapMemory(msect->virt);																								// And now free everything
...
Status ShmMapSection(UIntPtr key, PUIntPtr ret) {
	...
	Status status = MmAllocAddress(L"Shared Memory", shm->pregions.length * MM_PAGE_SIZE, SHM_EXIST_FLAGS, &msect->virt);			// Alloc the virtual address
	...
	if (!ListAdd(&PsCurrentProcess->shm_mapped_sections, msect)) {																	// Now add it to the process mapped shm sections list
		MmUnmapMemory(msect->virt);																									// Failed...
		MemFree((UIntPtr)msect);
...
Status ShmUnmapSection(UIntPtr key) {
	...
	MmUnmapMemory(msect->virt);																										// Now, free the virtual memory (and if this is the last reference, also free the physical memory)
