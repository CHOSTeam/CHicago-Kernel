...
Void PhysMem::Initialize(UInt8 *Start, UIntPtr KernelStart, UIntPtr KernelEnd, UIntPtr MaxAddress,
	...
	/* First, setup some of the basic fields, including the InInit field, that indicates that the we were
	 * called, but the FinishInitialization functions isn't haven't been called (that is, we can call
	 * InitializeRegion). */
	...
	/* Now, we need to initialize the region list, and the reference list, we need to do this before setting
	 * the kernel end, as we're going to put those lists after the current kernel end.
	 * There is one very important thing, we expect the arch-entry to already have mapped those lists onto
	 * virtual memory, on x86(-32/-64), the bootloader does it for us, on other architectures, it's probably
	 * going to be a bit different, but it needs to be already mapped. */
	
	Regions = (PhysMem::Region*)Start;
	References = &Start[RegionCount * sizeof(PhysMem::Region)];
	
	/* Now, let's clear the region list and the reference list (set all the entries on the region list to be
	 * already used, and all the entries on the references list to have 0 references). */
	...
	/* Everything that is left now is initializing all the regions, but were not going to do that, the caller
	 * should do it, as it should have some type of memory map, for determining the free regions. */
}

Void PhysMem::InitializeRegion(UIntPtr Address) {
	/* This function can only be called when we're initializing the physical memory manager, calling at any
	 * other time should just return without doing anything. */
...
Void PhysMem::FinishInitialization(Void) {
	/* Just as in the other function, we should only be called when we're initializing the PMM (actually, when
	 * we just finished initializing it). */
...
Status PhysMem::AllocNonContig(UIntPtr Count, UIntPtr *Out) {
	/* We need to manually check the two paramenter here, as we're going to alloc page-by-page, and we're going
	 * to return multiple addresses, instead of returning only a single one that points to the start of a bunch
...
Status PhysMem::FreeNonContig(UIntPtr *Pages, UIntPtr Count) {
	/* This is the same as AllocNonContig, but instead of calling AllocSingle, we call FreeSingle, and we don't
	 * need to save anything after doing that. */
...
Status PhysMem::ReferenceSingle(UIntPtr Page, UIntPtr &Out) {
	/* For referencing single pages, we can just check if we need to alloc a new page (if we do we self call us
	 * with said new allocated page), and increase the reference counter for the page. */
...
UIntPtr PhysMem::CountFreePages(UIntPtr BitMap, UIntPtr Start, UIntPtr End) {
	/* As each bit of the bitmap is one physical memory page, to count the amount of free pages we just need to
	 * count how many (consecutive) bits are NOT set. */
...
Status PhysMem::FindFreePages(UIntPtr BitMap, UIntPtr Count, UIntPtr &Out) {
	/* Each unset bit is one free page, for finding consective free pages, we can iterate through the bits of
	 * the bitmap and search for free bits. */
	
	UIntPtr bc = sizeof(UIntPtr) * 8;
	
	for (UIntPtr i = 0; i < bc;) {
		if (BitMap == UINTPTR_MAX || bc - i < Count) {
			/* If all the remaining bits are set, or if there aren't enough bits left, there is really nothing
			 * we can do, we're out of memory (in this bitmap). */
			
			return Status::OutOfMemory;
		} else if (BitMap == 0) {
			/* If all the remaining bytes are free, we can just return the current position, as we already
			 * checked if we have enough bits! */
...
Status PhysMem::AllocInt(UIntPtr Count, UIntPtr &Out) {
	...
	/* Now, we can just go through each region, and each bitmap in each region, and try to find the
	 * consecutive free pages that we were asked for. */
	
	for (UIntPtr i = 0; i < RegionCount; i++) {
		if (Regions[i].Free < Count) {
			/* This variable exists so we don't need to check a region that WILL NOT have enough bits in any
			 * of the bitmaps. */
		...
		for (UIntPtr j = 0; j < (1 << MM_REGION_SIZE_SHIFT) /
			...
			if (FindFreePages(Regions[i].Pages[j], Count, bit) == Status::Success) {
				/* Oh, we actually found enough free pages! we need to set all the bits that we're going to
				 * use, increase the used bytes variable, and calculate the return value. */
