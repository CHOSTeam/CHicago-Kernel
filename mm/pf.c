...
// Last edited on January 25 of 2020, at 14:21 BRT
...
static Status MmPageFaultDoAOR(UIntPtr addr, UInt32 prot) {
...
static Status MmPageFaultDoCOW(UIntPtr addr, UInt32 prot) {
	if (addr == 0 || (addr & ~MM_PAGE_MASK) != 0) {																								// Sanity check
		return STATUS_INVALID_ARG;
	}
...
static Status MmPageFaultDoMapFile(PFsNode file, UIntPtr addr, UIntPtr off, UInt32 prot, Boolean alloc, Boolean free) {
	if (file == Null || addr == 0 || (addr & ~MM_PAGE_MASK) != 0 || (off & ~MM_PAGE_MASK) != 0 || off >= file->length) {						// Sanity check
		return STATUS_INVALID_ARG;
	} else if (alloc) {																															// Should we alloc first?
...
Status MmPageFaultHandler(PMmRegion region, PVoid arch, UIntPtr addr, UInt32 flags, UInt8 reason) {
	...
	case MM_PF_WRITE_ON_READONLY: {																												// Copy on Write (COW)
		status = MmPageFaultDoCOW(addr & MM_PAGE_MASK, flags & MM_MAP_PROT);
		break;
	}
	default: {
	...
	} else if (region != Null && region->file != Null && reason == MM_PF_READWRITE_ON_NON_PRESENT) {											// Should we map (part of) a file into this page?
		status =  MmPageFaultDoMapFile(region->file, addr, (addr & MM_PAGE_MASK) - region->key.start, flags & MM_MAP_PROT, alloc, free);		// Yeah, handle it in another function...
	}
	
	if (status != STATUS_SUCCESS) {
		return MmPageFaultCrash(arch, addr, reason);																							// Just another check...
	}
